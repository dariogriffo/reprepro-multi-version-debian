...
With `--height HEIGHT[%]`, reprepro-multi-version will start below the cursor with the given height.
...
```sh
git ls-files | reprepro-multi-version --style full \
    --border --padding 1,2 \
    --border-label ' Demo ' --input-label ' Input ' --header-label ' File Type ' \
    --preview 'reprepro-multi-version-preview.sh {}' \
    --bind 'result:transform-list-label:
        if [[ -z $QUERY ]]; then
          echo " $MATCH_COUNT items "
        else
          echo " $MATCH_COUNT matches for [$QUERY] "
        fi
        ' \
...
### Customizing reprepro-multi-version options for completion
